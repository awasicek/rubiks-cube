...
const Face = props => {
    ...
    const centerRightTile = centerTiles.find(tile => tile["x-coordinate"] === 1);
    const centerMiddleTile = centerTiles.find(tile => tile["x-coordinate"] === 0);
    const centerLeftTile = centerTiles.find(tile => tile["x-coordinate"] === -1);

    const bottomTiles = tiles.filter(tile => tile["y-coordinate"] === -1);
    const bottomRightTile = bottomTiles.find(tile => tile["x-coordinate"] === 1);
    const bottomMiddleTile = bottomTiles.find(tile => tile["x-coordinate"] === 0);
    const bottomLeftTile = bottomTiles.find(tile => tile["x-coordinate"] === -1);
